...
- NFTablesSync: Firewall synchronization
...
class BlacklistManager:
    ...
    def __init__(self, whitelist_manager: WhitelistManager, 
        ...
        self.nft_sync = NFTablesSync(self.config, whitelist_manager, geolocation_manager)
    ...
    def update_blacklists(self, detections: List[DetectionResult]):
        ...
        if new_ips_info:
            self.logger.warning(f"🚨 Detected {len(new_ips_info)} new malicious IPs")
            ...
            if new_to_blacklist > 0:
                self.logger.info(f"✅ {new_to_blacklist} IPs imported from NFTables")
    ...
    def add_manual_ip(self, ip_str: str, reason: str = "Manually added", 
        ...
        try:
            ...
            if self.whitelist_manager.is_whitelisted(ip):
                self.logger.error(f"❌ Cannot blacklist {ip_str} - it's in whitelist")
                return False
    ...
    def show_blacklist(self):
        ...
        """
        print("🚨 ENHANCED BLACKLIST INFORMATION")
        print("=" * 120)
    ...
    def _update_blacklist_file(self, filename: str, new_ips_info: Dict):
        """
