...
| `HTTP_ERROR_4XX` | Status code 400-499 | N/A (status code range) |
| `HTTP_ERROR_5XX` | Status code 500-599 | N/A (status code range) |
...
## FTP Parser (ftp.py) [NEW]

**Parses:** FTP server logs (vsftpd, ProFTPD, Pure-FTPd)

### EventTypes Generated

| EventType | Trigger Condition | Pattern Source |
|-----------|------------------|----------------|
| `FTP_ATTACK` | Failed login or authentication error | `ftp.yaml: failed_login` |
...
ProFTPD:
  2025-12-23 10:15:42 USER admin (Login failed): No such user from [1.2.3.4]

Pure-FTPd:
  Dec 23 10:15:42 server pure-ftpd: (?@1.2.3.4) [WARNING] Authentication failed
```

---

## SMTP Parser (smtp.py) [NEW]

**Parses:** Mail server logs (Postfix, Sendmail, Exim)

### EventTypes Generated

| EventType | Trigger Condition | Pattern Source |
|-----------|------------------|----------------|
| `SMTP_ATTACK` | Failed authentication, relay attempts, abuse | `smtp.yaml: smtp_attacks` |
...
Postfix:
  ...
  Dec 23 10:15:43 mail postfix/smtpd[12346]: NOQUEUE: reject: RCPT from unknown[1.2.3.4]: Relay access denied

Exim:
  2025-12-23 10:15:42 authenticator failed for (test) [1.2.3.4]: 535 Incorrect authentication data
```
